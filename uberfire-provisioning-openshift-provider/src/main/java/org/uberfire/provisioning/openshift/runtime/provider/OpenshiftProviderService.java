...
public class OpenshiftProviderService implements ProviderService {

    private DefaultKubernetesClient kubernetes;
    private OpenshiftProvider provider;
    ...
    public org.uberfire.provisioning.runtime.Runtime create( RuntimeConfiguration runtimeConfig ) throws ProvisioningException {
        ...
        try {
            ...
            return new OpenshiftRuntime( serviceName, runtimeConfig, provider );
        } catch ( Exception ex ) {
            ex.printStackTrace();
            throw new ProvisioningException( "Error provisioning to Kubernetes: " + ex.getMessage() );
        }
